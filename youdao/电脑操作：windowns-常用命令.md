...
![image](F6F1CDAC75BC42E7852BCFF0CB97541F)
...
- 基于字符的应用程序和工具提供了==非图形界面的运行环境==
- 执行命令，并在屏幕回显 `MS-DOS` 风格的字符

![image](02953F4887144AC2852D2A835421319E)
...
>
>![Windows 键](9BB9B16A8DAF4F19918F59C00F826CCE)
...
- `win+x` 选择管理员方式运行 `powershell`，输入：
    - `dism.exe /Online /Cleanup-Image /AnalyzeComponentStore`清理系统组件存储垃圾！
    - `dism.exe /online /Cleanup-Image /StartComponentCleanup` 清理 `WinSxS`组件存储
...
## 附录：使用 windows 你可能还需知道的 ...

### Windows 激活
1. 如何搜索到最新 windows 10 永久激活密钥/序列号？
> Google 搜索`win10 神key`，然后按照时间筛选最近1周内收录发布的信息，成功率较高。

![image](D1864A94A73540C98DC6D0CBAD888C4A)

### 新 Windows 使用记录
1. 新购买的 Magic book pro 键盘没有 `Home`, `End`, `Page UP`, `Page DOWN` 这几个键 ==> 可以通过用 `Fn 键`+ `方向键` 到同样的功能
> - Home 键 = Fn + 左
> - End 键 = Fn + 右方向
> - PageUP = Fn + 上方向
> - PageDOWN = Fn + 下方向
> - 向前 Delete(Mac) = Fn + delete 键

直到后来仔细的观摩了一番同事的 MacBook，原来 MacBook 一直就是这样使用，还被鄙视的问了句“你唔噶？”...T娘的...都怪**穷**惹得祸.

![image](54B40B05F68A42C3AD19032120B13AA0)

### Windows 10 触摸板手势

![Windows 10 触摸板手势](https://cdn.jsdelivr.net/gh/MinsonLee/minsonlee.github.io/images/pig/20220414094351.png)

## Windows 系统如何切换至 UTF-8 编码

`控制面板》区域` 或者 `Windows 系统设置》时间和语言》语言》管理语言设置` 打开区域设置管理，然后选择“更改系统区域设置”，勾选“Beta版: 使用 Unicode UTF-8 提供全球语言支持” 即可
