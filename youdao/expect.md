...
proc expect_success { SERVER } {
    switch $SERVER {
        dev_web {
            # ERC 测试环境 web 机
            #set USER "developer_su"
            set USER "lms"
            set HOST "10.4.133.200"
            set PORT "63305"
            #set PWD "J2X0MO6AuKVRoRfB"
            set PWD "erc_dev_lms"
        }
        dev_mysql {
        # ERC-DEV-MySQL
            set USER "developer"
            set HOST "10.4.133.201"
            set PORT "63305"
            set PWD "InGSUFKF3dgsEYcB4jSz"
        }
        dev_mc {
        # ERC-DEV-MC&Sphinx
            set USER "developer"
            set HOST "10.4.133.202"
            set PORT "63305"
            set PWD "InGSUFKF3dgsEYcB4jSz"
        }
        dev_proxy {
        # ERC-DEV-NginxProxy
            set USER "developer"
            set HOST "10.4.133.204"
            set PORT "63305"
            set PWD "InGSUFKF3dgsEYcB4jSz"
        }
        erc_jumpserver {
            # ERC-ONLINE-JumpServer
            # 主 备：10.2.242.91
            set USER "limingshuang"
            set HOST "10.2.242.90"
            set PORT "2222"
            set PWD "ZZchelms1994~"
        }
        default {
            puts "Unknown Server!"
            exit
    ...
    # 启动进程执行 ssh 连接到服务器
    spawn ssh -o HostKeyAlgorithms=+ssh-rsa -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o CheckHostIP=false -l $USER $HOST -p $PORT;
    expect {
        "*yes/no*" { send "yes\r"; exp_continue; }
        "*password:*" { send "$PWD\r"; }
    }
